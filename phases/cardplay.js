...
(function () {
  ...
  /** Match engine + assets (gameUtils.cardNames are lowercase snake_case). */
  function canonicalRisqueCardName(raw) {
    if (raw == null || raw === "") return "";
    var s = String(raw).trim().toLowerCase().replace(/\s+/g, "_");
    var ok = window.gameUtils && Array.isArray(window.gameUtils.cardNames) ? window.gameUtils.cardNames : null;
    if (ok && ok.indexOf(s) !== -1) return s;
    if (ok) {
      var compact = s.replace(/_/g, "");
      var j;
      for (j = 0; j < ok.length; j += 1) {
        if (String(ok[j] || "").replace(/_/g, "") === compact) return ok[j];
      }
    }
    return s;
  }
      ...
      player.cards = (Array.isArray(player.cards) ? player.cards : [])
        .map(function (card) {
          if (typeof card === "string") {
            var n0 = canonicalRisqueCardName(card);
            return n0 ? { name: n0, id: generateUUID() } : null;
          }
          if (card && card.name) {
            var n1 = canonicalRisqueCardName(card.name);
            if (!n1) return null;
            var id0 = card.id != null && String(card.id) !== "" ? String(card.id) : generateUUID();
            if (seenIds[id0]) id0 = generateUUID();
            seenIds[id0] = true;
            return { name: n1, id: id0 };
          }
          return null;
        })
        .filter(Boolean);
  ...
  function removeCardFromHandAndDiscard(player, cardId, cardName) {
    var ok = removeCardFromHand(player, cardId, cardName);
    if (
      ok &&
      cardName &&
      window.gameUtils &&
      ...
      window.gameUtils.risqueDiscardCardNames(window.gameState, [cardName]);
    }
    return ok;
  }
  ...
  console.log("[Cardplay] Module loaded at", new Date().toLocaleString());
    ...
    /** Bumped on RST / new book so in-flight async book work exits cleanly (isResetting clears too early). */
    let cardplayBookEffectGeneration = 0;
    ...
    function logToStorage(message, data) {
      ...
      if (logs.length > CARDPLAY_LOG_MAX_ENTRIES) {
        logs = logs.slice(logs.length - CARDPLAY_LOG_MAX_ENTRIES);
      }
      function tryWrite(arr) {
        localStorage.setItem("gameLogs", JSON.stringify(arr));
      }
      try {
        tryWrite(logs);
      } catch (err) {
        try {
          tryWrite(logs.slice(-40));
        } catch (err2) {
          try {
            localStorage.removeItem("gameLogs");
            tryWrite([logEntry]);
          } catch (err3) {
            console.warn("[Cardplay] Log write skipped:", err3);
          }
        }
      }
      console.log(logEntry, data || "");
      if (typeof mountOpts.onLog === "function") {
        try {
          mountOpts.onLog(logEntry, data);
        } catch (eOn) {
          /* Host log hook must not break cardplay (e.g. quota on same storage key). */
        }
      }
    }
     ...
     * Compact staging: 3 book cards or 1 single-card pick are *ready* to PROCESS (apply play), not a hold.
     */
    function cardplayConfirmShouldBeHeld() {
      ...
      if (isCardplayHudCompact()) {
        if (isBookSelectionMode && selectedCards.length === 3) return false;
        if (isIndividualSelectionMode && selectedCards.length === 1) return false;
      }
      if (selectedCards.length > 0) return true;
      return false;
    }

    /** Compact toolbar: PROCESS runs staged book/single; CONFIRM commits plays / advances after processing. */
    function cardplayCompactPrimaryBarLabel() {
      if (!isCardplayHudCompact()) return "CONFIRM";
      if (!processingBook && isBookSelectionMode && selectedCards.length === 3) return "PROCESS";
      if (!processingBook && isIndividualSelectionMode && selectedCards.length === 1) return "PROCESS";
      return "CONFIRM";
    }
    ...
    function cardplayInit() {
      ...
      currentPlayer.cards = currentPlayer.cards
        .map(function (card) {
          if (typeof card === "string") {
            var ns = canonicalRisqueCardName(card);
            return ns ? { name: ns, id: generateUUID() } : null;
          }
          if (card && card.name) {
            var nm = canonicalRisqueCardName(card.name);
            if (!nm) return null;
            var idc = card.id != null && String(card.id) !== "" ? String(card.id) : generateUUID();
            if (seenCur[idc]) idc = generateUUID();
            seenCur[idc] = true;
            return { name: nm, id: idc };
          }
          return null;
        })
        .filter(Boolean);
      ...
      if (hasCards) {
        if (useHud) {
          const playedIdsAtMount = new Set();
          playedCards.forEach(function (pc) {
            (pc.cards || []).forEach(function (c) {
              if (c && c.id) playedIdsAtMount.add(c.id);
            });
          });
          cardHtmlCompact = buildCompactHandGridHtml(currentPlayer, playedIdsAtMount, new Set());
        } else {
          unplayedCards.forEach((card, index) => {
            var leg = canonicalRisqueCardName(card && card.name);
            var legOk =
              window.gameUtils &&
              Array.isArray(window.gameUtils.cardNames) &&
              leg &&
              window.gameUtils.cardNames.indexOf(leg) !== -1;
            if (index < cardPositions.length && legOk && card.id) {
              const pos = cardPositions[index];
              const alt = leg.replace(/_/g, " ");
              cardHtml += `
              <img class="card" src="assets/images/Cards/${String(leg || "").toUpperCase()}.webp" style="left: ${pos.x}px; top: ${pos.y}px; width: ${pos.width}px; height: ${pos.height}px;" data-card="${leg}" data-id="${card.id}" alt="${alt} card">
            `;
            }
          });
        }
      ...
      const cardplayBodyHtmlHud =
        ...
        '<div id="cardplay-book-pill" class="risque-book-pill" hidden role="status" aria-hidden="true">' +
      ...
      const buttons = [
        ...
        { id: "next-phase-button", enabled: false, handler: handleCompactConfirmOrNext },
        { id: "cardplay-panel-confirm-btn", enabled: false, handler: handleCompactConfirmOrNext }
      ];
    ...
    /**
     * Compact HUD hand: one DOM slot per physical card in player.cards order.
     * Staged picks stay in selection — invisible placeholders keep neighbors from sliding (whack-a-mole).
     */
    function buildCompactHandGridHtml(currentPlayer, playedIds, stagedIds) {
      var html = "";
      if (!currentPlayer || !window.gameUtils) return html;
      var namesOk =
        window.gameUtils && Array.isArray(window.gameUtils.cardNames) ? window.gameUtils.cardNames : [];
      (currentPlayer.cards || []).forEach(function (card) {
        if (!card || !card.id) return;
        var cn = canonicalRisqueCardName(card.name);
        if (!cn || namesOk.indexOf(cn) === -1) return;
        if (playedIds.has(card.id)) return;
        if (stagedIds.has(card.id)) {
          html +=
            '<div class="cardplay-hand-slot cardplay-hand-slot--staged" aria-hidden="true"></div>';
          return;
        }
        html +=
    ...
    /**
     * Staging strip: active picks from selectedCards; after a book or single card is fully processed
     * but not yet committed to the public recap, show those cards dimmed from playedCards.
     */
    function getCompactStagingCardRefs() {
      if (selectedCards.length > 0) {
        return { refs: selectedCards, dimmed: false };
      }
      for (var i = playedCards.length - 1; i >= 0; i--) {
        var pc = playedCards[i];
        if (!pc || pc.action !== "book" || pc.confirmed || !pc.cards || !pc.cards.length) continue;
        var refs = pc.cards.map(function (c) {
          var cardName = typeof c.card === "string" ? c.card : c.card && c.card.name;
          return { card: cardName, id: c.id };
        });
        return { refs: refs, dimmed: true };
      }
      var indRefs = [];
      for (var k = 0; k < playedCards.length; k++) {
        var pc1 = playedCards[k];
        if (!pc1 || pc1.confirmed || !pc1.cards || pc1.cards.length !== 1 || pc1.action === "book") continue;
        var c0 = pc1.cards[0];
        if (!c0 || !c0.id) continue;
        var cn = typeof c0.card === "string" ? c0.card : c0.card && c0.card.name;
        if (!cn) continue;
        indRefs.push({ card: cn, id: c0.id });
      }
      if (indRefs.length > 0) {
        return { refs: indRefs, dimmed: true };
      }
      return { refs: [], dimmed: false };
    }

    function rebuildCardplayHandAndStaging() {
      const handGrid = document.getElementById("cardplay-card-grid");
      const stagingGrid = document.getElementById("cardplay-staging-grid");
      const stagingWrap = document.getElementById("cardplay-staging-wrap");
      if (!handGrid || !window.gameState || !window.gameUtils) return;
      const currentPlayer = window.gameState.players.find(function (p) {
        return p.name === window.gameState.currentPlayer;
      });
      if (!currentPlayer) return;
      ...
      var stagingInfo = getCompactStagingCardRefs();
      const stagedIds = new Set(stagingInfo.refs.map(function (s) { return s.id; }));
      handGrid.innerHTML = buildCompactHandGridHtml(currentPlayer, playedIds, stagedIds);
      wireCardplayCardClicks(handGrid);
      if (stagingGrid) {
        let stHtml = "";
        stagingInfo.refs.forEach(function (sc) {
          if (!sc || !sc.id || !sc.card) return;
          const alt = sc.card.replace(/_/g, " ");
          var dim = stagingInfo.dimmed ? " played" : "";
          stHtml +=
            '<img class="card cardplay-compact-card cardplay-staging-card' +
            dim +
            '" src="assets/images/Cards/' +
            String(sc.card || "").toUpperCase() +
            '.webp" data-card="' +
            sc.card +
            '" data-id="' +
            sc.id +
    ...
    function refreshValidBookReminder() {
      var pill = document.getElementById("cardplay-book-pill");
      if (!pill) return;
      if (window.__risqueCardplayHostIncomeOnly) {
        pill.hidden = true;
    ...
    async function toggleCardSelection(img) {
      ...
      if (isCardplayHudCompact()) {
        ...
        if (!isBookSelectionMode && !isIndividualSelectionMode) {
          setCardplayError("Tap CARD or BOOK first.");
          return;
        }
      ...
      if (img.classList.contains("selected")) {
        ...
        selectedCards = selectedCards.filter(function (c) { return c.id !== cardId; });
        setCardplayError("");
        updateSummaryDisplay();
        checkCardStatus();
        return;
    ...
    async function validateAndProcessBook() {
      ...
      cardplayBookEffectGeneration++;
      bookPlayedCard.__bookEffectGen = cardplayBookEffectGeneration;
      processingBook = true;
      ...
      try {
        ...
        logToStorage("Book processing threw", {
          message: eBk && eBk.message ? eBk.message : String(eBk)
        });
        setCardplayError("Book processing failed — tap RST to restore, or reload card play.");
      }
    }
    async function processBookCardEffect(bookPlayedCard) {
      ...
      if (currentBookCardIndex >= selectedCards.length) {
        ...
        const currentPlayer = window.gameState.players.find(p => p.name === window.gameState.currentPlayer);
        const unplayedCards = getUnplayedCards();
        /* Stay idle after a book so CARD / BOOK / CONFIRM can all be used; do not force book selection mode. */
        if (unplayedCards.length >= 3 && canFormValidBook(unplayedCards)) {
          setCardplayError(
            `Book processed — tap BOOK for another book, CARD for one card, or CONFIRM when ready. (${unplayedCards.length} cards left)`
          );
    ...
    async function processWildcardCard(card, cardId) {
      ...
      if (!action) {
        ...
        selectedCards = [];
        document.querySelectorAll(".card").forEach(function (cEl) {
      ...
      const currentPlayer = window.gameState.players.find(p => p.name === window.gameState.currentPlayer);
      if (action === 'territory') {
        const allNames = Array.isArray(window.gameUtils.cardNames) ? window.gameUtils.cardNames : [];
        if (!allNames.length) {
          isIndividualSelectionMode = false;
          setCardplayError("Card catalog missing — reload the game.");
          selectedCards = [];
          document.querySelectorAll(".card").forEach(function (cEl) {
            cEl.classList.remove("selected");
          });
          if (isCardplayHudCompact()) rebuildCardplayHandAndStaging();
          checkCardStatus();
          return;
        }
        const territoryOptions = allNames
          .filter(name => !['wildcard1', 'wildcard2'].includes(name))
          .map(name => ({ value: name, text: name.toUpperCase() }));
        const territory = await showPopup('Assign wildcard to a territory:', territoryOptions);
        const terrCanon = territory ? canonicalRisqueCardName(territory) : "";
        if (!territory || allNames.indexOf(terrCanon) === -1) {
          if (window.isResetting) return;
          isIndividualSelectionMode = false;
        ...
        await processIndividualCardEffect(card, terrCanon, cardId);
      } else if (action === 'aerial') {
        if (window.isResetting) return;
        /* Same as processIndividualCardEffect: leaving individual mode stuck true with empty staging
         * disables compact CONFIRM (nextOkC only allows idle when !inInd). */
        isIndividualSelectionMode = false;
        if (window.gameUtils && typeof window.gameUtils.addAerialAttackUses === "function") {
          window.gameUtils.addAerialAttackUses(window.gameState, 1);
        } else {
    ...
    function loadCards() {
      ...
      if (isCardplayHudCompact()) {
        ...
        cardElements.forEach(function (card) {
          ...
          if (!unplayedCards.some(function (c) { return c.id === cardId; })) {
    ...
    function checkCardStatus() {
      ...
      const useCompact = isCardplayHudCompact();
      const inBook = isBookSelectionMode;
      const inInd = isIndividualSelectionMode;
      const idle = !inBook && !inInd && !processingBook;
      const staging = selectedCards.length;
      var holdConfirm = cardplayConfirmShouldBeHeld();

      if (useCompact) {
        if (playCardButton) {
          var playOk = unplayedCards.length > 0 && !processingBook && idle;
          if (inBook || (inInd && staging > 0)) {
            playOk = false;
          }
          playCardButton.disabled = !playOk;
          playCardButton.classList.toggle("enabled", playOk);
        }
        if (selectCardsButton) {
          var bookOk = canFormBook && !processingBook && idle;
          if (inBook || (inInd && staging > 0)) {
            bookOk = false;
          }
          selectCardsButton.disabled = !bookOk;
          selectCardsButton.classList.toggle("enabled", bookOk);
        }
        if (resetButton) {
          var resetOk =
            staging > 0 ||
          ...
          resetButton.disabled = !resetOk;
          resetButton.classList.toggle("enabled", resetOk);
        }
        if (nextPhaseButton) {
          var nextOkC = false;
          if (!holdConfirm) {
            if (inBook && staging === 3) {
              ...
              nextOkC = hasCardActions && unplayedCards.length <= 4;
            }
          }
          nextPhaseButton.disabled = !nextOkC;
          nextPhaseButton.classList.toggle("enabled", nextOkC);
        }
      } else {
        if (playCardButton) {
        ...
        if (nextPhaseButton) {
          var nextOkL = hasCardActions && unplayedCards.length <= 4 && !holdConfirm;
          nextPhaseButton.disabled = !nextOkL;
          nextPhaseButton.classList.toggle("enabled", nextOkL);
        }
      }
      ...
      if (skipIncBtn) {
        ...
        if (useCompact) {
          ...
          skipOk = unplayedCards.length <= 4 && !holdConfirm && !hasUnconfirmedActions;
        }
        ...
        skipIncBtn.title = skipOk
          ...
          : unplayedCards.length > 4
            ...
            : hasUnconfirmedActions
              ...
              : useCompact && (!idle || staging > 0)
                ? "Skip is only available before you start CARD or BOOK selection."
    ...
    function cardplayHasUnconfirmedPlays() {
      return playedCards.some(function (pc) {
        return pc && !pc.confirmed;
      });
    }

    function abortStaleBookEffect(bookPlayedCard) {
      var g = bookPlayedCard && bookPlayedCard.__bookEffectGen;
      if (g == null || g === cardplayBookEffectGeneration) return false;
      processingBook = false;
      currentBookCardIndex = -1;
    ...
    function performFullCardplayMountSnapshotReset() {
      if (!initialGameState || !Array.isArray(initialGameState.players)) {
        setCardplayError("Cannot reset — missing snapshot. Reload card play or use Load game.");
        return;
      }
      ...
      try {
        window.gameState.deck = JSON.parse(JSON.stringify(initialGameState.deck || []));
      } catch (eDeck) {
        window.gameState.deck = Array.isArray(initialGameState.deck) ? initialGameState.deck.slice() : [];
      }
      try {
        window.gameState.discardPile = JSON.parse(JSON.stringify(initialGameState.discardPile || []));
      } catch (eDisc) {
        window.gameState.discardPile = Array.isArray(initialGameState.discardPile)
          ? initialGameState.discardPile.slice()
          : [];
      }
      ...
      setCardplayError("Actions reset — restored to start of this card play.");
      if (isCardplayHudCompact()) {
      ...
      var cp = window.gameState.players.find(function (p) {
        return p.name === window.gameState.currentPlayer;
      });
      logToStorage(
        "Game state reset",
        cp ? { player: cp.name, cards: cp.cards, territories: cp.territories } : {}
      );
    }
    ...
    async function handleReset() {
      ...
      if (cardplayHasUnconfirmedPlays()) {
        window.isResetting = true;
        try {
          performFullCardplayMountSnapshotReset();
        } finally {
      ...
      if (isCardplayHudCompact()) {
        ...
        if (
          (isBookSelectionMode || isIndividualSelectionMode) &&
          playedCards.length === 0 &&
          !processingBook
        ) {
          window.isResetting = true;
    ...
    function publishCardplayRecapToPublicMirror() {
      ...
      try {
        ...
        localStorage.removeItem(AERIAL_COUNTER_DECISION_KEY);
        localStorage.removeItem(AERIAL_DECISION_READY_KEY);
      } catch (eClrAck) {}
          ...
          if (hasAerialStep) {
            try {
              localStorage.setItem(
                ...
                JSON.stringify({ seq: Number(nextSeq), ready: true, at: Date.now() })
              );
            } catch (eAerialReady) {
              /* ignore */
            }
    ...
    async function handleCompactConfirmOrNext() {
      ...
      if (isCardplayHudCompact()) {
        ...
        if (isIndividualSelectionMode && selectedCards.length === 1) {
          const sc0 = selectedCards[0];
          if (["wildcard1", "wildcard2"].includes(sc0.card)) {
            await processWildcardCard(sc0.card, sc0.id);
          } else {
            await processIndividualCardEffect(sc0.card, sc0.card, sc0.id);
          }
          return;
        }
      }
